...
def _headers_map(scope: Scope) -> dict[str, str]:
    ...
    if not isinstance(raw_headers, list):
        return {}
    return {
        bytes(key).decode("latin-1"): bytes(value).decode("latin-1") for key, value in raw_headers
    }
...
class ASGIOAuthApp:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if response.body is not None and "content-type" not in {
            key.lower() for key, _ in (response.headers or {}).items()
        }:
    ...
    def _dispatch(self, scope: Scope, body: bytes) -> HttpResponse:
        ...
        routes: dict[tuple[str, str], Callable[[NormalizedRequestContext], HttpResponse]] = {
            ...
            ): self._adapter.handle_webauthn_authenticate_start,
            (
                "POST",
                "/webauthn/authenticate/finish",
            ): self._adapter.handle_webauthn_authenticate_finish,
        ...
        if (method, path) == ("GET", "/.well-known/openid-configuration"):
            return apply_runtime_security_headers(
                self._adapter.handle_openid_configuration(issuer=self._issuer),
                allow_caching=True,
            )
        if method == "OPTIONS":
            return apply_runtime_security_headers(
                preflight_response(request, policy=self._origin_policy)
            )
        handler = routes.get((method, path))
        if handler is None:
            return apply_runtime_security_headers(
                HttpResponse(status=404, body={"error": "not_found"})
            )
        return apply_runtime_security_headers(
            call_with_origin_policy(request, handler, policy=self._origin_policy),
            allow_caching=(method, path) == ("GET", "/.well-known/jwks.json"),
        )
...
def create_asgi_app(
    *,
    ...
    return ASGIOAuthApp(adapter=adapter, issuer=issuer, origin_policy=origin_policy)
