...
from astraauth.core.authorization.exceptions import RoleNotFoundError
...
from astraauth.core.authorization.store import AssignmentStore, PolicyStore, RoleStore
...
class AuthorizationEngine:
    ...
    def authorize_context(self, context: AuthorizationContext) -> AuthorizationDecision | None:
        ...
        if step_ups:
            required_acr = max(
                match.required_acr or context.required_acr or 1 for match in step_ups
            )
    ...
    def _evaluate_policies(self, context: AuthorizationContext) -> tuple[PolicyMatch, ...]:
        assert self._policies is not None  # nosec B101
