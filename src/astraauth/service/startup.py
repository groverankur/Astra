...
from astraauth.core.oauth.password import MultiSchemePasswordVerifier, hash_password
from astraauth.core.security import ThrottleStoreSnapshot
from astraauth.core.token.token_manager import TokenKeyManager
from astraauth.service.factory import AstraAuthService, build_service
from astraauth.service.observability import (
...
_TOKEN_KEYS_FILENAME = "token-keys.json"  # nosec B105
_STATE_BUNDLE_VERSION = 1
_ENCRYPTED_BOOTSTRAP_EXPORT_VERSION = 1
...
class RuntimeSecurityReport:
    home: Path
    runtime_throttle: ThrottleStoreSnapshot
    plugin_audit_log_path: Path
    plugin_audit_record_count: int
    recent_plugin_audit_records: tuple[PluginAuditLogRecord, ...]
...
def _hash_bootstrap_secret(secret: str) -> str:
...
def export_token_key_state(*, output_path: Path, home: Path | None = None) -> Path:
    ...
    payload = encrypt_runtime_mapping(manager.dump_private_state(), home=home)
    write_private_text(output_path, json.dumps(payload, indent=2))
...
def import_token_key_state(
    *, input_path: Path, config: AuthConfig | None = None, home: Path | None = None
) -> Path:
    target_home = home or DEFAULT_ASTRAAUTH_HOME
...
def build_service_from_home(*, home: Path | None = None) -> AstraAuthService:
    ...
    service = build_service(
        config=config, token_manager=token_manager, observability_home=target_home
    )
...
def _plugin_audit_log_path(*, home: Path | None = None) -> Path:
    return (home or DEFAULT_ASTRAAUTH_HOME) / "logs" / "plugin-runtime-audit.jsonl"


def _plugin_audit_log_record_from_payload(payload: dict[str, Any]) -> PluginAuditLogRecord:
    return PluginAuditLogRecord(
        timestamp=datetime.fromisoformat(str(payload["timestamp"])).replace(tzinfo=UTC),
        ...
        error_classification=str(payload["error_classification"])
        if payload.get("error_classification") is not None
        else None,
        message=str(payload["message"]) if payload.get("message") is not None else None,
    )


def _load_plugin_audit_log_records(*, home: Path | None = None) -> tuple[PluginAuditLogRecord, ...]:
    path = _plugin_audit_log_path(home=home)
    if not path.exists():
        return ()
    records: list[PluginAuditLogRecord] = []
    for line in path.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        payload = json.loads(line)
        if isinstance(payload, dict):
            records.append(_plugin_audit_log_record_from_payload(payload))
    return tuple(records)


def runtime_security_report(*, home: Path | None = None) -> RuntimeSecurityReport:
    target_home = home or DEFAULT_ASTRAAUTH_HOME
    service = build_service_from_home(home=target_home)
    recent_records = _load_plugin_audit_log_records(home=target_home)
    return RuntimeSecurityReport(
        home=target_home,
        runtime_throttle=service.throttle_store.snapshot(),
        plugin_audit_log_path=_plugin_audit_log_path(home=target_home),
        plugin_audit_record_count=len(recent_records),
        recent_plugin_audit_records=recent_records[-20:][::-1],
    )
...
def _encrypted_export_fernet(*, home: Path | None = None) -> Fernet:
    target_home = home or DEFAULT_ASTRAAUTH_HOME
...
def _encrypted_bootstrap_export_payload(
    ...
    *,
    ...
    manifest_hash = hashlib.sha256(plaintext).hexdigest()
    now = datetime.now(tz=UTC)
    if recipient_public_key_path is not None:
        data_key = Fernet.generate_key()
        encrypted_payload = Fernet(data_key).encrypt(plaintext).decode("utf-8")
        public_key = _load_rsa_public_key(recipient_public_key_path)
        encrypted_key = public_key.encrypt(
            data_key,
            padding.OAEP(
                mgf=padding.MGF1(algorithm=hashes.SHA256()),
                algorithm=hashes.SHA256(),
                label=b"astraauth-bootstrap-export-v1",
            ),
        )
        return {
            "artifact_type": "astraauth.encrypted_bootstrap_manifest",
            "version": _ENCRYPTED_BOOTSTRAP_EXPORT_VERSION,
    ...
    encrypted = _encrypted_export_fernet(home=home).encrypt(plaintext).decode("utf-8")
    return {
        "artifact_type": "astraauth.encrypted_bootstrap_manifest",
        "version": _ENCRYPTED_BOOTSTRAP_EXPORT_VERSION,
        "algorithm": "fernet",
...
def _load_rsa_public_key(path: Path) -> rsa.RSAPublicKey:
    ...
    if not isinstance(key, rsa.RSAPublicKey):
        raise ValueError("recipient public key must be an RSA public key")
    return key


def _load_rsa_private_key(path: Path) -> rsa.RSAPrivateKey:
    key = serialization.load_pem_private_key(path.read_bytes(), password=None)
    if not isinstance(key, rsa.RSAPrivateKey):
        raise ValueError("recipient private key must be an RSA private key")
    return key


def _decrypt_recipient_bootstrap_payload(
    *,
    encrypted_payload: str,
    encrypted_key: str,
    recipient_private_key_path: Path,
) -> bytes:
    private_key = _load_rsa_private_key(recipient_private_key_path)
    ...
    try:
        return Fernet(data_key).decrypt(encrypted_payload.encode("utf-8"))
    except InvalidToken as exc:
        raise ValueError("recipient-encrypted bootstrap export payload is invalid") from exc


def _decrypt_local_bootstrap_payload(
    *,
    encrypted_payload: str,
    home: Path | None = None,
) -> bytes:
    try:
        return _encrypted_export_fernet(home=home).decrypt(encrypted_payload.encode("utf-8"))
    except (FileNotFoundError, InvalidToken) as exc:
        raise ValueError(
            "encrypted bootstrap export cannot be decrypted with this runtime home"
        ) from exc


def _decrypt_bootstrap_export_payload(
    payload: Any,
    *,
    home: Path | None = None,
    ...
    if not isinstance(payload, dict):
        return payload
    if payload.get("artifact_type") != "astraauth.encrypted_bootstrap_manifest":
        return payload
    encrypted_payload = payload.get("encrypted_payload")
    if not isinstance(encrypted_payload, str):
        raise ValueError("encrypted bootstrap export missing encrypted_payload")
    algorithm = str(payload.get("algorithm", "fernet"))
    if algorithm == "rsa-oaep-sha256+fernet":
        encrypted_key = payload.get("encrypted_key")
        if not isinstance(encrypted_key, str):
            raise ValueError("recipient-encrypted bootstrap export missing encrypted_key")
        if recipient_private_key_path is None:
            raise ValueError("recipient private key is required for this bootstrap export")
        decrypted = _decrypt_recipient_bootstrap_payload(
            encrypted_payload=encrypted_payload,
            encrypted_key=encrypted_key,
            recipient_private_key_path=recipient_private_key_path,
        )
    else:
        decrypted = _decrypt_local_bootstrap_payload(encrypted_payload=encrypted_payload, home=home)
    decrypted_payload = json.loads(decrypted.decode("utf-8"))
    if not isinstance(decrypted_payload, dict):
        raise ValueError("encrypted bootstrap export payload must decrypt to an object")
    expected_hash = payload.get("manifest_hash")
    actual_hash = hashlib.sha256(
        json.dumps(decrypted_payload, sort_keys=True).encode("utf-8")
    ).hexdigest()
    if expected_hash is not None and not hmac.compare_digest(str(expected_hash), actual_hash):
        raise ValueError("encrypted bootstrap export hash mismatch")
...
def _bootstrap_manifest_from_payload(payload: Any) -> BootstrapManifest:
    ...
    return BootstrapManifest(
        admins=tuple(admins), setup_tokens=tuple(tokens), setup_locked=setup_locked
    )
...
def authenticate_operator_admin(
    *,
    ...
    if (target_home / "config.json").exists():
        return _authenticate_operator_admin_from_service(
    ...
    return _authenticate_operator_admin_from_manifest(
        target_home=target_home,
        manifest=manifest,
        tenant_id=tenant_id,
        username=username,
...
def _authenticate_operator_admin_from_service(
    *,
    target_home: Path,
    ...
    for admin in manifest.admins:
        if admin.tenant_id != tenant_id or admin.username != username:
            continue
        if verifier.needs_rehash(stored_password_hash=admin.password_hash):
            _save_upgraded_bootstrap_admin_hash(
                manifest=manifest,
                target_home=target_home,
                subject_id=admin.subject_id,
    ...
    if not _roles_have_admin_access(
        service=service, tenant_id=tenant_id, roles=roles, home=target_home
    ):
...
def _authenticate_operator_admin_from_manifest(
    *,
    target_home: Path,
    manifest: BootstrapManifest,
    tenant_id: str,
    ...
    for admin in manifest.admins:
        if admin.tenant_id != tenant_id or admin.username != username:
            continue
        valid, upgraded_hash = verifier.verify_and_update(
            provided_password=password,
            stored_password_hash=admin.password_hash,
        )
        if not valid:
            raise ValueError("invalid_admin_credentials")
        if upgraded_hash is not None and upgraded_hash != admin.password_hash:
            _save_upgraded_bootstrap_admin_hash(
                manifest=manifest,
                target_home=target_home,
                subject_id=admin.subject_id,
...
def _save_upgraded_bootstrap_admin_hash(
    *,
    manifest: BootstrapManifest,
    target_home: Path,
    subject_id: str,
        ...
        BootstrapManifest(
            admins=tuple(
                BootstrapAdminRecord(
                    ...
                    else item.password_hash,
                    subject_id=item.subject_id,
                    role_name=item.role_name,
                    client_id=item.client_id,
                    email=item.email,
                )
                for item in manifest.admins
            ),
...
def list_plugin_audit_records(
    *,
    ...
    return tuple(
        ...
        for record in _load_plugin_audit_log_records(home=home)
        if (tenant_id is None or record.tenant_id == tenant_id)
        and (plugin_name is None or record.plugin_name == plugin_name)
        and (execution_type is None or record.execution_type == execution_type)
...
def record_admin_action(
    *,
    ...
    target_home = home or DEFAULT_ASTRAAUTH_HOME
    config = (
        load_auth_config(home=target_home)
        if (target_home / "config.json").exists()
        else AuthConfig()
    )
...
def export_runtime_config(
    *,
    ...
    config = load_auth_config(home=home)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    write_private_text(output_path, json.dumps(config.model_dump(mode="json"), indent=2))
    return output_path
...
def export_bootstrap_manifest(
    *,
    ...
    if unsafe_plaintext and recipient_public_key_path is not None:
        raise ValueError("unsafe_plaintext cannot be combined with recipient_public_key_path")
    manifest = load_bootstrap_manifest(home=home)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    payload = _bootstrap_payload(manifest)
    if not unsafe_plaintext:
        payload = _encrypted_bootstrap_export_payload(
            payload,
            home=home,
...
def create_bootstrap_setup_token(
    *,
    ...
    record = BootstrapSetupTokenRecord(
        token_id=str(uuid4()),
        token_hash=_hash_bootstrap_secret(plain_token),
        created_at=now,
    ...
    *,
    ...
    for record in manifest.setup_tokens:
        ...
        if record.expires_at < now:
            continue
        if hmac.compare_digest(record.token_hash, _hash_bootstrap_secret(token)):
            return record
...
def import_bootstrap_manifest(
    *,
    ...
    raw = json.loads(input_path.read_text(encoding="utf-8"))
    imported = _bootstrap_manifest_from_payload(
        _decrypt_bootstrap_export_payload(
            raw,
            home=home,
            recipient_private_key_path=recipient_private_key_path,
        )
...
def export_runtime_state_bundle(
    *,
    output_path: Path,
    home: Path | None = None,
    unsafe_plaintext_bootstrap: bool = False,
    recipient_public_key_path: Path | None = None,
) -> Path:
    if unsafe_plaintext_bootstrap and recipient_public_key_path is not None:
        raise ValueError(
            "unsafe_plaintext_bootstrap cannot be combined with recipient_public_key_path"
        )
    config = load_auth_config(home=home)
    manifest = load_bootstrap_manifest(home=home)
    bootstrap_payload = _bootstrap_payload(manifest)
    if not unsafe_plaintext_bootstrap:
        bootstrap_payload = _encrypted_bootstrap_export_payload(
            bootstrap_payload,
            home=home,
            recipient_public_key_path=recipient_public_key_path,
        )
...
def import_runtime_state_bundle(
    *,
    ...
    imported_manifest = _bootstrap_manifest_from_payload(
        _decrypt_bootstrap_export_payload(
            bootstrap_payload,
            home=target_home,
            recipient_private_key_path=recipient_private_key_path,
...
def _load_admin_action_audit_records(
    *, home: Path | None = None
) -> tuple[AdminActionAuditRecord, ...]:
    ...
    return tuple(
        AdminActionAuditRecord(
            ...
            actor_subject_id=str(item["actor_subject_id"])
            if item.get("actor_subject_id") is not None
            else None,
            actor_tenant_id=str(item["actor_tenant_id"])
            if item.get("actor_tenant_id") is not None
            else None,
            actor_username=str(item["actor_username"])
...
class RuntimeDiagnosticsReport:
    ...
    settings_key_metadata_exists: bool
    settings_key_id: str | None
    settings_key_stale: bool
    weak_permission_paths: tuple[str, ...]
    config_encrypted: bool
...
def verify_backup_artifact(
    *,
    ...
    if isinstance(raw, dict) and raw.get("bundle_version") == _STATE_BUNDLE_VERSION:
        ...
        config = AuthConfig.model_validate(config_payload)
        manifest = _bootstrap_manifest_from_payload(
            _decrypt_bootstrap_export_payload(
                bootstrap_payload,
                home=target_home,
                recipient_private_key_path=recipient_private_key_path,
        ...
        if (target_home / "config.json").exists():
            ...
            matches_runtime = current_config.model_dump(mode="json") == config.model_dump(
                mode="json"
    ...
    if isinstance(raw, dict) and (
        isinstance(raw.get("admins"), list)
        or raw.get("artifact_type") == "astraauth.encrypted_bootstrap_manifest"
    ):
        manifest = _bootstrap_manifest_from_payload(
            _decrypt_bootstrap_export_payload(
                raw,
        ...
        if (target_home / _BOOTSTRAP_FILENAME).exists():
            bootstrap_matches_runtime = _bootstrap_payload(
                load_bootstrap_manifest(home=target_home)
            ) == _bootstrap_payload(manifest)
        details.append(f"bootstrap_admins={len(manifest.admins)}")
    ...
    try:
        ...
        if (target_home / "config.json").exists():
            config_matches_runtime = load_auth_config(home=target_home).model_dump(
                mode="json"
...
def runtime_diagnostics_report(*, home: Path | None = None) -> RuntimeDiagnosticsReport:
    ...
    key_inspection = inspect_settings_key(home=target_home)
    config_encrypted = False
    if config_path.exists():
        raw_config = json.loads(config_path.read_text(encoding="utf-8"))
        config_encrypted = runtime_mapping_contains_encrypted_values(raw_config)
        if not config_encrypted:
            warnings.append("unencrypted_config_values")

    if not settings_key_path.exists():
        warnings.append("missing_settings_key")
    elif not key_inspection.metadata_exists:
        warnings.append("missing_settings_key_metadata")
    if key_inspection.stale:
        warnings.append("stale_settings_key")
    if key_inspection.weak_permissions:
        warnings.append("weak_local_permissions")
    if not key_inspection.permissions_supported:
        warnings.append("windows_acl_review_required")
    if not token_keys_path.exists():
        warnings.append("missing_token_keys")
    if key_inspection.metadata is not None:
        details.append(f"settings_key_id={key_inspection.metadata.key_id}")
    ...
    return RuntimeDiagnosticsReport(
        ...
        settings_key_metadata_exists=key_inspection.metadata_exists,
        settings_key_id=(
            key_inspection.metadata.key_id if key_inspection.metadata is not None else None
        ),
        settings_key_stale=key_inspection.stale,
        weak_permission_paths=key_inspection.weak_permissions,
...
def rotate_config_settings_key(*, home: Path | None = None) -> str:
    target_home = home or DEFAULT_ASTRAAUTH_HOME
    metadata = rotate_settings_key(home=target_home)
    config = load_auth_config(home=target_home)
    record_metric(config=config, name="config_key.rotations", home=target_home)
    record_event(
        config=config,
        event_type="config_key.rotated",
        status="succeeded",
        home=target_home,
