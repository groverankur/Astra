...
from astraauth.service.factory import HTTPOIDCCodeExchangeClient, HTTPOIDCMetadataClient


def test_http_oidc_exchange_client_validates_id_token_against_jwks() -> None:
    key = jwk.RSAKey.generate_key(2048)
    keyset = jwk.KeySet.import_key_set(
        jwk.KeySetSerialization(keys=[key.as_dict(is_private=False)])
    )
...
def test_http_oidc_metadata_client_rejects_non_http_discovery_url() -> None:
    client = HTTPOIDCMetadataClient()

    with pytest.raises(OIDCProtocolError, match="unsupported_oidc_url_scheme"):
        client.fetch_metadata(discovery_url="file:///tmp/openid-configuration.json")


def test_http_oidc_exchange_client_rejects_non_http_jwks_url() -> None:
    client = HTTPOIDCCodeExchangeClient()

    with pytest.raises(OIDCProtocolError, match="unsupported_oidc_url_scheme"):
        client._fetch_keyset("file:///tmp/jwks.json")
