...
from astraauth_core.persistence.relational import compile_sql, create_sync_database, upsert_sql
...
def _audit_from_row(row: dict[str, Any]) -> FederationAuditRecord:
    return FederationAuditRecord(
        ...
        external_subject=str(row["external_subject"]) if row["external_subject"] is not None else None,
