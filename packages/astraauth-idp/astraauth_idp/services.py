...
from astraauth_idp.models import (
...
from astraauth_idp.store import (
...
def discover_provider_metadata(
    *,
    ...
    discovery_url = provider.discovery_url or f"{provider.issuer.rstrip('/')}/.well-known/openid-configuration"
    ...
    metadata = OIDCProviderMetadata(
        ...
        userinfo_endpoint=str(raw["userinfo_endpoint"]) if raw.get("userinfo_endpoint") is not None else None,
        scopes_supported=_tupleify_strings(raw.get("scopes_supported")),
        response_types_supported=_tupleify_strings(raw.get("response_types_supported"), default=("code",)),
        subject_types_supported=_tupleify_strings(raw.get("subject_types_supported"), default=("public",)),
...
def _transform_claim(value: object, mapping: ClaimAttributeMapping) -> str | int | float | bool | None:
