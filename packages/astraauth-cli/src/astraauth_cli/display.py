...
from astraauth.service.redaction import redact_value
...
def print_json(payload: object, *, redact: bool = True) -> None:
    safe_payload = redact_value(payload) if redact else payload
    print(json.dumps(safe_payload, indent=4))
...
def print_persistence(payload: Mapping[str, object]) -> None:
    ...
    if not isinstance(safe_payload, Mapping):
        return
    stores = safe_payload.get("stores", [])
    print_text(f"home={safe_payload['home']}")
...
def print_bootstrap(payload: Mapping[str, object]) -> None:
    ...
    for item in admins:
        ...
        if admin is not None:
            print_text(
                f"{_field(admin, 'tenant_id')}:{_field(admin, 'username')} "
                f"role={_field(admin, 'role_name')} client={_field(admin, 'client_id')}"
            )
...
def print_oidc_audit(payload: Mapping[str, object]) -> None:
    ...
    for item in records:
        ...
        if record is not None:
            print_text(
                f"{_field(record, 'created_at')} {_field(record, 'status')} "
                f"{_field(record, 'event_type')} provider={_field(record, 'provider_id')}"
            )
...
def print_admin_audit(payload: Mapping[str, object]) -> None:
    ...
    for item in records:
        ...
        if record is not None:
            print_text(
                f"{_field(record, 'created_at')} {_field(record, 'status')} "
                f"{_field(record, 'event_type')} actor={_field(record, 'actor_username')}"
            )
...
def print_security_report(payload: Mapping[str, object]) -> None:
    runtime_throttle = payload.get("runtime_throttle", {})
    admin_ui_throttle = payload.get("admin_ui_throttle", {})
    print_kv_lines(
        [
            ("home", payload["home"]),
            ("plugin_audit_log_path", payload["plugin_audit_log_path"]),
            ("plugin_audit_record_count", payload["plugin_audit_record_count"]),
            (
                "runtime_throttle_storage",
                runtime_throttle.get("storage_kind")
                if isinstance(runtime_throttle, Mapping)
                else None,
            ),
            (
                "runtime_blocked_buckets",
                runtime_throttle.get("blocked_bucket_count")
                if isinstance(runtime_throttle, Mapping)
                else None,
            ),
            (
                "admin_ui_throttle_storage",
                admin_ui_throttle.get("storage_kind")
                if isinstance(admin_ui_throttle, Mapping)
                else None,
            ),
            (
                "admin_ui_blocked_buckets",
                admin_ui_throttle.get("blocked_bucket_count")
                if isinstance(admin_ui_throttle, Mapping)
                else None,
            ),
        ]
    )
    recent = payload.get("recent_plugin_audit_records", [])
    if isinstance(recent, list):
        for record in recent[:10]:
            if isinstance(record, Mapping):
                print_text(
                    "plugin_audit="
                    f"{record.get('timestamp')} {record.get('plugin_name')} "
                    f"{record.get('execution_type')} {record.get('status')} {record.get('duration_ms')}ms"
                )
