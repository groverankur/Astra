...
from astraauth.service import runtime_inventory_report
...
def run_admin_ui(
    *,
    ...
    show_security: Callable[..., None],
) -> None:
    inventory = runtime_inventory_report(home=home)
    render_admin_summary(
        home=inventory.home, environment=inventory.environment, issuer=inventory.issuer
    )
    choices = [
    ...
    while True:
        ...
        if action == "health":
            ...
            continue
        if action == "security":
            show_security(home=home, as_json=False)
            continue
