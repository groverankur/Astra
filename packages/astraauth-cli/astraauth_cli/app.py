...
def state_export_command(
    ...
    print_text(f"state_export={export_runtime_state_bundle(home=home, output_path=output)}")
...
def backup_verify_command(
    ...
    report = verify_backup_artifact(home=home, input_path=input_path)
    payload = {
...
def bootstrap_export_command(
    ...
    print_text(f"bootstrap_export={export_bootstrap_manifest(home=home, output_path=output)}")
...
def bootstrap_import_command(
    ...
    print_text(f"bootstrap_import={import_bootstrap_manifest(home=home, input_path=input_path, merge=merge)}")
...
def admin_ui_command(
    ...
    resolved_home = resolve_home(home)
    if not no_tui and run_textual_admin_ui(home=resolved_home):
        return
    run_admin_ui(
        home=resolved_home,
