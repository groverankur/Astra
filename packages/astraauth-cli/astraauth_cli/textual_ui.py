# mypy: disable-error-code="import-not-found,misc,valid-type"
from __future__ import annotations

from pathlib import Path
from typing import Any, cast

from astraauth_core.config import DEFAULT_ASTRAAUTH_HOME
...
def _run_textual_ui(*, mode: str, home: Path | None = None) -> bool:  # noqa: C901
    ...
    class AstraConsoleApp(app_cls[None]):
        ...
        #bootstrap-pretty, #jwks-pretty, #audit-pretty {
            height: 16;
        }
        """
        ...
        def compose(self) -> Any:
            ...
            with tabbed_content_cls(initial="wizard" if self._mode_name == "wizard" else "dashboard"):
                ...
                with tab_pane_cls("Wizard", id="wizard"):
                    with vertical_scroll_cls(classes="pane"):
                        ...
                        yield input_cls(value="dev", placeholder="Environment", id="cfg-environment")
                        yield input_cls(value="sqlite", placeholder="Persistence backend", id="cfg-backend")
                        yield input_cls(value="https://auth.local", placeholder="Issuer", id="cfg-issuer")
                        yield button_cls("Initialize or reset config", id="cfg-init", variant="primary")
                        yield static_cls("Create bootstrap admin", classes="section")
                        yield input_cls(value="tenant-1", placeholder="Tenant ID", id="admin-tenant")
                        yield input_cls(value="admin", placeholder="Admin username", id="admin-username")
                        yield input_cls(value="change-me", placeholder="Admin password", password=True, id="admin-password")
                        yield input_cls(value="admin@example.com", placeholder="Admin email", id="admin-email")
                ...
                with tab_pane_cls("OIDC Audit", id="audit"):
                    with vertical_scroll_cls(classes="pane"):
                        yield input_cls(value="tenant-1", placeholder="Tenant ID", id="audit-tenant")
                        yield input_cls(value="", placeholder="Provider ID (optional)", id="audit-provider")
        ...
        def _refresh_jwks(self) -> None:
            try:
                self._pretty("jwks-pretty").update({"keys": export_public_jwks(home=self._home_path)})
            except Exception as exc:
        ...
        def _set_status(self, message: str, *, error: bool = False) -> None:
            ...
            cast(Any, self.query_one("#status")).update(f"{prefix} {message}")
