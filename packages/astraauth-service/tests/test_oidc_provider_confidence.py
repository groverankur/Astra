...
from astraauth_idp import ClaimAttributeMapping, GroupRoleMapping, OIDCProviderConfig
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self) -> None:  # noqa: N802
                ...
                if parsed.path == "/oauth2/userinfo":
                    ...
                    token = auth.removeprefix("Bearer ") if isinstance(auth, str) and auth.startswith("Bearer ") else None
...
def test_oidc_provider_fixture_end_to_end_session_issuance() -> None:
    ...
    try:
        ...
        assignment = svc.assignments.get_assignments("idp:tenant-1:oidc-corp:ext-user-1", "tenant-1")
...
def test_oidc_provider_fixture_rejects_bad_issuer_audience_and_nonce() -> None:
    ...
    try:
        ...
        scenarios: list[tuple[str, str | None, tuple[str, ...] | None, str | None, str]] = [
            ("auth-code-issuer", f"{provider.base_url}/wrong", None, None, "id_token_issuer_mismatch"),
