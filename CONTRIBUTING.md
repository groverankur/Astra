...
uv sync --all-groups
uv run ruff check .
uv run ty check packages examples --python .venv
...
- Ruff, ty, and pytest should pass on the changed surface at minimum. The repo-level ty config currently treats migration-heavy diagnostics as warnings while the repo completes the checker transition.
