"""Astra Litestar Sample App — Polished E2E Example.

Demonstrates: login/logout, session management, RBAC-gated routes,
and step-up MFA on sensitive actions using Litestar.

Run:
    uv run python examples/litestar_app.py
...
logger = logging.getLogger("astra.example.litestar")
...
.topbar h1{font-size:1.1rem;font-weight:700;color:#a78bfa;letter-spacing:.5px}
...
.card h2{font-size:1rem;font-weight:600;color:#c4b5fd;margin-bottom:14px}
...
.btn-primary{background:#7c3aed;color:#fff}.btn-danger{background:#dc2626;color:#fff}.btn-ghost{background:#334155;color:#e2e8f0}
...
def _session_info(request: Request) -> dict | None:  # type: ignore[type-arg]
...
def _page(title: str, body: str, session: dict | None = None) -> LitestarResponse[str]:
    nav = user_pill = ""
    if session:
    ...
    html = f"""<!doctype html><html><head><meta charset=utf-8><title>{title} — Astra Litestar</title>{CSS}</head><body>
<div class="topbar"><h1>💜 Astra · Litestar</h1><div style="display:flex;align-items:center;gap:16px">{nav}{user_pill}</div></div>
<div class="container">{body}</div></body></html>"""
    return LitestarResponse(content=html, status_code=200, media_type="text/html")
...
# ---------------------------------------------------------------------------
@get("/")
async def index(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    if _session_info(request):
        return Redirect(path="/dashboard")  # type: ignore[return-value]
...
@post("/login")
async def login(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    ...
    if REQUIRE_LOGIN_MFA:
        redirect_resp = Redirect(path="/mfa?next=/dashboard&login_flow=true")
        redirect_resp.set_cookie(
            Cookie(key="astra_temp_session", value=sid, httponly=True, samesite="lax")
        )
        return redirect_resp  # type: ignore[return-value]

    redirect_resp = Redirect(path="/dashboard")
    redirect_resp.set_cookie(Cookie(key="astra_session", value=sid, httponly=True, samesite="lax"))
    return redirect_resp  # type: ignore[return-value]


@get("/oidc/mock/login")
async def oidc_mock_login(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    req = NormalizedRequestContext(
        http_method="POST",
    ...
    if REQUIRE_LOGIN_MFA:
        response = Redirect(path="/mfa?next=/dashboard&login_flow=true")
        response.set_cookie(
            Cookie(key="astra_temp_session", value=sid, httponly=True, samesite="lax")
        )
    ...
    response.set_cookie(Cookie(key="astra_session", value=sid, httponly=True, samesite="lax"))
    return response  # type: ignore[return-value]


@get("/webauthn/mock/login")
async def webauthn_mock_login(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    req = NormalizedRequestContext(
        http_method="POST",
        request_path="/token",
        query_params={},
...
@get("/signout")
async def app_logout(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    sid = request.cookies.get("astra_session")
    if sid:
        req = NormalizedRequestContext(
            http_method="POST",
    ...
    redirect_resp.delete_cookie("astra_session")
    redirect_resp.delete_cookie("astra_temp_session")
    return redirect_resp  # type: ignore[return-value]


@post("/admin/settings")
async def update_settings(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    session = _session_info(request)
    if not session:
        return Redirect(path="/")  # type: ignore[return-value]
    ...
    REQUIRE_LOGIN_MFA = form.get("mfa_enabled") == "true"
    return Redirect(path="/dashboard")  # type: ignore[return-value]


@get("/dashboard")
async def dashboard(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    session = _session_info(request)
    if not session:
        return Redirect(path="/")  # type: ignore[return-value]
...
@get("/documents")
async def documents_list(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    session = _session_info(request)
    if not session:
        return Redirect(path="/")  # type: ignore[return-value]
...
@get("/documents/{doc_id:str}/delete")
async def delete_doc(doc_id: str, request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    global DOCUMENTS
    session = _session_info(request)
    if not session:
        return Redirect(path="/")  # type: ignore[return-value]
    ...
    if not doc:
        return Redirect(path="/documents")  # type: ignore[return-value]
...
@get("/mfa")
async def mfa_page(
    request: Request, next: str = "/dashboard", login_flow: bool = False
) -> LitestarResponse[str]:  # type: ignore[type-arg]
...
@post("/mfa/verify-otp")
async def app_mfa_verify(request: Request) -> LitestarResponse[str]:  # type: ignore[type-arg]
    form = await request.form()
    login_flow = form.get("login_flow") == "true"
    target_cookie = "astra_temp_session" if login_flow else "astra_session"
    ...
    otp = str(form.get("otp", ""))
    next_url = str(form.get("next", "/dashboard"))
    if otp == "123456":
        sess.upgrade_authentication(target_acr=2, methods={"email_otp"})
        store.save(sess)
        response = Redirect(path=next_url)
        if login_flow:
            response.set_cookie(
                Cookie(key="astra_session", value=sid, httponly=True, samesite="lax")
            )
...
@get("/api/documents")
async def api_documents(request: Request) -> LitestarResponse[dict[str, Any]]:  # type: ignore[type-arg]
    ...
    return LitestarResponse(content={"documents": filtered_docs}, status_code=200)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Astra Litestar Sample App")
    print(f"Open → {BASE_URL}")
    print("Accounts: alice/alice-password (admin)  bob/bob-password (user)")
    print("=" * 60)
    uvicorn.run("litestar_app:app", host="127.0.0.1", port=PORT, reload=False)
