"""Astra Robyn Sample App — Polished E2E Example.

Demonstrates: login/logout, session management, RBAC-gated routes,
and step-up MFA on sensitive actions using Robyn (Rust-backed async web framework).

Run:
    uv run python examples/robyn_app.py
...
logger = logging.getLogger("astra.example.robyn")
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .topbar h1{font-size:1.1rem;font-weight:700;color:#f43f5e;letter-spacing:.5px}
  ...
  .card h2{font-size:1rem;font-weight:600;color:#fda4af;margin-bottom:14px}
  ...
  .btn-primary{background:#e11d48;color:#fff}.btn-danger{background:#dc2626;color:#fff}.btn-ghost{background:#334155;color:#e2e8f0}
...
def _get_cookie(request: Request, name: str) -> str | None:
    cookie_str = request.headers.get("cookie") or request.headers.get("Cookie") or ""
...
def _parse_form_data(request: Request) -> dict[str, str]:
    ...
    return {str(k): str(v[0]) for k, v in parsed.items() if v}


def _session_info(request: Request) -> dict[str, Any] | None:
    sid = _get_cookie(request, "astra_session")
    if not sid:
        return None
...
def _page(title: str, body: str, session: dict | None = None) -> Response:
    nav = user_pill = ""
    if session:
    ...
    html = f"""<!doctype html><html><head><meta charset=utf-8><title>{title} — Astra Robyn</title>{CSS}</head><body>
<div class="topbar"><h1>🦀 Astra · Robyn</h1><div style="display:flex;align-items:center;gap:16px">{nav}{user_pill}</div></div>
<div class="container">{body}</div></body></html>"""
    return Response(status_code=200, headers={"Content-Type": "text/html"}, description=html)


# ---------------------------------------------------------------------------
# Robyn App Initialization
# ---------------------------------------------------------------------------
app = Robyn(__file__)
...
async def index(request: Request) -> Response:
    if _session_info(request):
        return Response(status_code=303, headers={"Location": "/dashboard"}, description="")
...
async def login(request: Request) -> Response:
    ...
    if REQUIRE_LOGIN_MFA:
        return Response(
            status_code=303,
            headers={
                "Location": "/mfa?next=/dashboard&login_flow=true",
                "Set-Cookie": f"astra_temp_session={sid}; HttpOnly; Path=/; SameSite=Lax",
            },
            description="",
        )
    ...
    return Response(
        ...
        headers={
            "Location": "/dashboard",
            "Set-Cookie": f"astra_session={sid}; HttpOnly; Path=/; SameSite=Lax",
        },
        description="",
    )
...
async def oidc_mock_login(request: Request) -> Response:
    req = NormalizedRequestContext(
        http_method="POST",
...
async def webauthn_mock_login(request: Request) -> Response:
    req = NormalizedRequestContext(
        http_method="POST",
        request_path="/token",
        query_params={},
...
async def logout(request: Request) -> Response:
    ...
    return Response(
        ...
        headers={
            "Location": "/",
            "Set-Cookie": "astra_session=; HttpOnly; Path=/; SameSite=Lax; Max-Age=0\r\nSet-Cookie: astra_temp_session=; HttpOnly; Path=/; SameSite=Lax; Max-Age=0",
        },
        description="",
    )
...
async def update_settings(request: Request) -> Response:
    ...
    if not session:
        return Response(status_code=303, headers={"Location": "/"}, description="")
...
async def delete_document(request: Request) -> Response:
    ...
    if not doc:
        return Response(status_code=303, headers={"Location": "/documents"}, description="")
...
async def mfa_page(request: Request) -> Response:
    login_flow = request.query_params.get("login_flow") == "true"
    target_cookie = "astra_temp_session" if login_flow else "astra_session"

    sid = _get_cookie(request, target_cookie)
    if not sid:
        return Response(status_code=303, headers={"Location": "/"}, description="")
...
async def mfa_verify(request: Request) -> Response:
    ...
    if otp == "123456":
        ...
        if login_flow:
            headers["Set-Cookie"] = (
                f"astra_session={sid}; HttpOnly; Path=/; SameSite=Lax\r\nSet-Cookie: astra_temp_session=; HttpOnly; Path=/; SameSite=Lax; Max-Age=0"
            )
        return Response(status_code=303, headers=headers, description="")
    return _page(
        "Invalid OTP",
...
async def api_documents(request: Request) -> Response:
    ...
    try:
        ...
        return Response(
            ...
            headers={"Content-Type": "application/json"},
            description=json.dumps(
                {"error": "unauthorized", "message": f"Token verification failed: {str(e)}"}
            ),
        )
    ...
    return Response(
        status_code=200,
        headers={"Content-Type": "application/json"},
        description=json.dumps({"documents": filtered_docs}),
    )
